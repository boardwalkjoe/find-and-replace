...
# DNS Name and IP Address Find and Replace Script
# Searches through files in a directory and replaces DNS names or IP addresses
...
# Validate old and new values (DNS names or IP addresses)
validate_input "$OLD_VALUE" "Old value" || log "WARN" "Old value may not be valid"
validate_input "$NEW_VALUE" "New value" || log "WARN" "New value may not be valid"

# Confirm operation if not dry run
if [[ $DRY_RUN == false ]]; then
    echo -n "Replace '$OLD_VALUE' with '$NEW_VALUE' in $DIRECTORY? (y/N): "
    read -r response
    if [[ ! "$response" =~ ^[Yy]$ ]]; then
...
find_and_replace "$DIRECTORY" "$OLD_VALUE" "$NEW_VALUE"
