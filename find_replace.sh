...
usage() {
    cat << EOF
Usage: $0 [OPTIONS] DIRECTORY OLD_DNS NEW_DNS

Find and replace DNS names in files within a directory.

ARGUMENTS:
    DIRECTORY    Directory to search
    OLD_DNS      DNS name to replace
    NEW_DNS      New DNS name
...
EXAMPLES:
    ...
    $0 /var/www old.example.com new.example.com --extensions "conf,txt,html"
    $0 . old.example.com new.example.com --backup --no-recursive
...
validate_dns() {
    ...
    if [[ ! $dns =~ $dns_regex ]]; then
        log "WARN" "'$dns' doesn't look like a valid DNS name"
        return 1
    fi
    return 0
}
